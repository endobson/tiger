   ...
   (case name
    ...
    ((readline)
     ...
     (let* ((string-ptr (llvm-alloca (llvm-pointer-type (llvm-int8-type))))
            (_ (llvm-call get-line string-ptr (llvm-null (llvm-pointer-type (llvm-int-type))) (llvm-load stdin-global)))
            (string-size (llvm-call strlen (llvm-load string-ptr)))
            (string (llvm-alloc-string string-size)))
      (llvm-store string-size (llvm-gep string 0 0))
      (llvm-call memcpy (llvm-gep string 0 1 0) (llvm-load string-ptr) string-size)
      (llvm-store (llvm-int 0 (llvm-int8-type)) (llvm-gep string 0 1 string-size))

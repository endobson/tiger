...
(define (add-external-functions stdin-global)


 (let ((i8* (llvm-pointer-type (llvm-int8-type))))
  (llvm-add-function (llvm-function-type (llvm-int-type) i8* #:varargs? #t) "printf")
  ...
  (for/hash (((name type) external-function-database))
   (case name
    ...
    ((itoa)
     ...
     (define snprintf (llvm-add-function (llvm-function-type (llvm-int-type) (llvm-pointer-type (llvm-int8-type)) (llvm-int-type) (llvm-pointer-type (llvm-int8-type)) #:varargs? #t) "snprintf"))
     (define function (llvm-add-function llvm-type "itoa_clos"))
     (llvm-set-position (llvm-add-block-to-function function))
     (define template (llvm-global-string-ptr "%d"))
     (let ((num-digits (llvm-call snprintf (llvm-null (llvm-pointer-type (llvm-int8-type))) 0 template (llvm-get-param 1))))
      (let ((string (llvm-alloc-string (llvm+ 1 num-digits))))
        (llvm-call snprintf (llvm-gep string 0 1 0) (llvm+ 1 num-digits) template (llvm-get-param 1))
        (llvm-store num-digits (llvm-gep string 0 0))
        (llvm-ret string)))
    ...
    ((readline)
     (define llvm-type (convert-function-type type))
     (define get-line (llvm-add-function (llvm-function-type (llvm-int-type) (llvm-pointer-type (llvm-pointer-type (llvm-int8-type))) (llvm-pointer-type (llvm-int-type)) (llvm-pointer-type (llvm-int8-type))) "getline"))

     (define free (llvm-add-function (llvm-function-type (llvm-void-type) (llvm-pointer-type (llvm-int8-type))) "free"))
     (define memcpy (llvm-get-named-function "memcpy"))
     (define printf (llvm-get-named-function "printf"))
     (define strlen (llvm-add-function (llvm-function-type (llvm-int-type) (llvm-pointer-type (llvm-int8-type))) "strlen"))
     (define function (llvm-add-function llvm-type "readline_clos"))
     (define template (llvm-global-string-ptr "%p\n"))
     (llvm-set-position (llvm-add-block-to-function function))
     (let* ((string-ptr (llvm-alloca (llvm-pointer-type (llvm-int8-type))))
            (int-ptr (llvm-alloca (llvm-int-type)))
            (_ (llvm-store (llvm-null (llvm-pointer-type (llvm-int8-type))) string-ptr))
            (string-size (llvm-call get-line string-ptr int-ptr (llvm-load stdin-global)))
            (last-char (llvm-load (llvm-gep (llvm-load string-ptr) (llvm- string-size 1))))
            (newline (llvm-= (llvm-int 10 (llvm-int8-type)) last-char))
            (string-size (llvm- (llvm+ string-size 1) (llvm-zext newline (llvm-int-type))))
            (string (llvm-alloc-string string-size)))
      (llvm-store (llvm- string-size 1) (llvm-gep string 0 0))
      (llvm-call memcpy (llvm-gep string 0 1 0) (llvm-load string-ptr) string-size)
      (llvm-store (llvm-int 0 (llvm-int8-type)) (llvm-gep string 0 1 (llvm- string-size 1)))
      (llvm-call free (llvm-load string-ptr))
      (llvm-ret string))
     (values name function))
    ...
    ((atoi)
     (define llvm-type (convert-function-type type))
     (define real-function (llvm-add-function (llvm-function-type (llvm-int-type) (llvm-pointer-type (llvm-int8-type))) "atoi"))
     (define function (llvm-add-function llvm-type "atoi_clos"))
     (llvm-set-position (llvm-add-block-to-function function))

     (llvm-ret (llvm-call real-function (llvm-gep (llvm-get-param 1) 0 1 0)))
     (values name function))
    ...
    ((getchar)
     ...
     (let ((char (llvm-call real-function)))
      ...
      (let ((string (llvm-alloc-string 2)))
       (llvm-store 1 (llvm-gep string 0 0))
       (llvm-store char (llvm-gep string 0 1 0))
       (llvm-store (llvm-int 0 (llvm-int8-type)) (llvm-gep string 0 1 1))
       (llvm-ret string)))
     (values name function))
    ((print)
     (define llvm-type (convert-function-type type))
    ...
    ((concat)
     ...
     (let ((source-string1 (llvm-get-param 1)) (source-string2 (llvm-get-param 2)))
      (let* ((size1 (llvm-load (llvm-gep source-string1 0 0)))
             ...
             (dest-string (llvm-alloc-string (llvm+ 1 size))))
       ...
       (llvm-store (llvm-int 0 (llvm-int8-type)) (llvm-gep dest-string 0 1 size))
    ...
    ((substring)
     ...
     (let ((source-string (llvm-get-param 1)) (first (llvm-get-param 2)) (n (llvm-get-param 3)))
      (let ((string-size (llvm-load (llvm-gep source-string 0 0))))
       ...
       (llvm-set-position error-block)
       (let ((null-string (llvm-alloc-string 1)))
        (llvm-store 0 (llvm-gep null-string 0 0))
        (llvm-store (llvm-int 0 (llvm-int8-type)) (llvm-gep null-string 0 1 0))
        (llvm-ret null-string))
       (llvm-set-position good-block)
       (let ((dest-string (llvm-alloc-string (llvm+ 1 n))))
        (llvm-store n (llvm-gep dest-string 0 0))
        (llvm-call memcpy (llvm-gep dest-string 0 1 0) (llvm-gep source-string 0 1 first) n)
        (llvm-store (llvm-int 0 (llvm-int8-type)) (llvm-gep dest-string 0 1 n))
        (llvm-ret dest-string))))
     (values name function))

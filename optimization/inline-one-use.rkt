    ...
    ((bind-rec funs body)
     (let-values (((inlined expr)
      (for/fold: : (values (Listof unique) bind-rec)
       ...
       ((fun-name : unique (map (inst car unique function) funs)))
        (let* ((fun-body (cdr (assert (assoc fun-name (bind-rec-functions expr)) pair?)))
               (do-inline (lambda () (values (cons fun-name inlined-functions) (assert (inline fun-name fun-body expr return-type) bind-rec?))))
               (do-inline-and-call (lambda () (values (cons fun-name inlined-functions) (assert (inline-and-call fun-name fun-body expr return-type) bind-rec?))))
               (dont-do-inline (lambda () (values inlined-functions expr))))
        (cond
         ((tail-called-once? fun-name expr) (do-inline))
         ((called-once? fun-name expr)
          (if (single-return-function? fun-body)
              (do-inline)
              (do-inline-and-call)))
         (else (dont-do-inline)))))))
      (match expr
